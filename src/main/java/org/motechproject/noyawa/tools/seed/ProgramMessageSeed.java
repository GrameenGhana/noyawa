...
public class ProgramMessageSeed extends Seed {
	...
	private List<RepeatingCampaignMessage> getAllCampaignMessages() {
        List<RepeatingCampaignMessage> messages = messageCampaigns.get(ProgramType.RONALD).messages();
        messages.addAll(messageCampaigns.get(ProgramType.KIKI).messages());
        messages.addAll(messageCampaigns.get(ProgramType.RITA).messages());
        return messages;
    }
